...
def query_hotpic(request):
    ...
    # PM_Shanghai = pd.read_csv('media/ShanghaiPM20100101_20151231.csv', parse_dates={'date': ['year', 'month', 'day', 'hour']},
    #                           date_parser=dateparse, index_col='date', keep_date_col=True)
    # # 清洗行列索引
    # PM_Shanghai.drop('No', axis=1, inplace=True)
    # PM_Shanghai = PM_Shanghai.loc['2013':'2015']
    # # 缺失值填充为三个PM的平均值
    # PM_Shanghai[['PM_Jingan', 'PM_US Post', 'PM_Xuhui']] = PM_Shanghai[
    #     ['PM_Jingan', 'PM_US Post', 'PM_Xuhui']].T.fillna(
    #     PM_Shanghai[['PM_Jingan', 'PM_US Post', 'PM_Xuhui']].T.mean()).T
    # # 缺失的值用前向填充法
    # PM_Shanghai[['PM_Jingan', 'PM_US Post', 'PM_Xuhui', 'TEMP', 'HUMI', 'DEWP']] = PM_Shanghai[
    #     ['PM_Jingan', 'PM_US Post', 'PM_Xuhui', 'TEMP', 'HUMI', 'DEWP']].fillna(method='ffill')
    # PM_Shanghai.DEWP['2013-3-11'].plot()
    # import base64
    # with open("media/126.jpg","rb") as f:
    #     base64_data = base64.b64encode(f.read())
    # src = str(base64_data)
    # print(type(src))
    src = 'https://cube.elemecdn.com/6/94/4d3ea53c084bad6931a56d5158a48jpeg.jpeg'
    return JsonResponse({'data':src})
